...
对本文档中的新特性有疑惑的地方可以参照[《ECMAScript 6 入门》](http://es6.ruanyifeng.com)与 [《learn-es2015》](https://babeljs.io/learn-es2015/)等。

虽然本文档是针对 ECMAScript 设计的，但是在使用各种基于 ECMAScript 扩展的语言时(如 JSX、TypeScript 等)，适用的部分也应尽量遵循本文档的约定。

<p class="tip">
   在使用vue、react等的项目中，尽量使用ESNext的语法。
</p>
...
不要使用3层及以上的解构,尽量使用解构减少中间变量，仅定义一个变量时不允许使用解构。
...
使用 `async/await` 代替 `generator` + `co`。
...
在运行环境中没有 `Promise` 时，将 `Promise` 的实现 `shim` 到 `global` 中。
...
使用 `babel` 做为预编译工具时，通过 `external-helpers` 减少生成文件的大小。

生成的代码在浏览器环境运行时，应生成 AMD 模块化代码。
